...
const enrichmentFile = path.join(projectRoot, 'src', 'data', 'product-enrichment.json')
...
function removeCjkParentheticals(value) {
...
async function main() {
  ...
  // Preserve editorial English enrichment across catalog regeneration. The
  // folder TXT files remain the source for inventory, prices, and images;
  // this optional overlay supplies the curated copy for matching IDs.
  try {
    ...
    for (const product of products) {
      const editorial = enrichmentById.get(product.id)
      if (!editorial) continue
      product.name = editorial.name || product.name
      product.tagline = editorial.tagline || product.tagline
      product.material = editorial.material || product.material
      product.energy = Array.isArray(editorial.energy) ? editorial.energy : product.energy
      product.benefits = Array.isArray(editorial.benefits) ? editorial.benefits : product.benefits
      product.howToWear = Array.isArray(editorial.howToWear) ? editorial.howToWear : product.howToWear
      product.specs = Array.isArray(editorial.specs) ? editorial.specs : product.specs
      product.careRitual = Array.isArray(editorial.careRitual) ? editorial.careRitual : product.careRitual
    }
    console.log(`Applied editorial enrichment to ${products.filter((product) => enrichmentById.has(product.id)).length} products.`)
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error
  }

  // The storefront is English-first. Preserve the English wording while
  // removing source-only Chinese glosses from customer-visible fields.
  for (const product of products) {
    for (const key of ['name', 'tagline', 'material', 'energy', 'benefits', 'howToWear', 'specs', 'careRitual']) {
      product[key] = Array.isArray(product[key])
        ? product[key].map(removeCjkParentheticals)
        : removeCjkParentheticals(product[key])
    }
  }
